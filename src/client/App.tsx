...
export function App({ identity }: { identity: IdentityClient }) {
  ...
  const run = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/runs', { method: 'POST', headers: { ...headers, 'content-type': 'application/json' }, body: JSON.stringify({ sessionId: crypto.randomUUID(), goal, ...(csvInput ? { csv: csvInput } : {}) }) })
