...
export class E2BProvider implements ExecutionProvider {
  ...
  async execute(id: string, input: ExecutionInput): Promise<ExecutionOutput> {
    ...
    await sandbox.files.write('/home/user/input.csv', input.csv)
    const code = `import csv, json\nfrom collections import defaultdict\nrows=list(csv.DictReader(open('/home/user/input.csv')))\nproducts=defaultdict(float); regions=defaultdict(lambda: [0,0,0])\nfor r in rows:\n products[r['product']]+=float(r['revenue']); x=regions[r['region']]; x[0]+=float(r['revenue']); x[1]+=float(r['satisfaction']); x[2]+=1\nbp=max(products,key=products.get); br=max(regions,key=lambda k: regions[k][0]); top=max(rows,key=lambda r:float(r['revenue']))\npatterns=[{'title':f'{bp} leads product revenue','evidence':f'{bp} generated $%s across the input.'%format(products[bp],',.0f')},{'title':f'{br} is the strongest region','evidence':f'{br} generated $%s with average satisfaction %.1f.'%(format(regions[br][0],',.0f'),regions[br][1]/regions[br][2])},{'title':'Top revenue record','evidence':f'{top["product"]} in {top["region"]} generated $%s with satisfaction {top["satisfaction"]}.'%format(float(top["revenue"]),',.0f')}]\nreport='# Vestren CSV Analysis\\n\\n## Top three patterns\\n\\n'+'\\n'.join([f'{i+1}. **{p["title"]}.** {p["evidence"]}' for i,p in enumerate(patterns)])+f'\\n\\n## Method\\nAnalyzed {len(rows)} rows inside an E2B sandbox.\\n'\nopen('/home/user/report.md','w').write(report)\nprint(json.dumps({'patterns':patterns,'report':report}))`
    ...
    const raw = result.logs.stdout.join(''); const parsed = JSON.parse(raw) as { report: string; patterns: ExecutionOutput['patterns'] }
